...
## 🔧 Patch v10.2 (21/04/2026) — Alerte structure Safari

**Problème rencontré** : Safari sur Mac traduit automatiquement les noms de dossiers en français quand tu dézippes le ZIP. Cela casse le build Vercel :
- `src/` devient `source/`
- `core/` devient `cœur/`
- `public/` devient `publique/`

**Solution** :
1. **Utilise Chrome ou Firefox** pour télécharger/dézipper (ou Terminal : `unzip ZIP_NAME.zip`)
2. Un fichier `STRUCTURE.md` a été ajouté à la racine du ZIP pour rappel
3. Si tu as déjà uploadé avec des noms traduits, **supprime** `source/`, `cœur/` et `publique/` de GitHub avant de re-uploader les bons dossiers

**Aucun changement de code dans v10.2** — même contenu que v10.1, juste une sécurité documentaire.
