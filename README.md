...
## 🎨 Patch v8.3 (20/04/2026 nuit) — Refonte UX "Matériels"

**Changement** : navigation en 2 niveaux au lieu d'une liste monolithique.

**Avant** : tous les chips catégories + filtres statuts + 223 outils sur la même page → cognitivement surchargé.

**Après** :
- **Écran 1** : grille 3 colonnes des 18 catégories, tuiles carrées avec icône proéminente + compteur d'outils + badge rouge si retards. Barre de recherche globale en haut qui traverse tout le parc (si utilisée → bascule en mode résultats avec catégorie affichée sur chaque ligne).
- **Écran 2** : clic sur une tuile → liste des outils de la catégorie, avec recherche locale et filtres statut. Bouton "← Catégories" pour revenir.

**Avantages** :
- Repérage visuel instantané par icône
- Recherche ciblée plus efficace dans un sous-ensemble
- Moins de scroll, plus calme visuellement
- Cohérent avec le pattern "tuiles" de la HomePage

**Fichier modifié** : `src/modules/parc-machines/ParcMateriels.jsx`
