...
import { collection, addDoc, updateDoc, doc, onSnapshot, query, getDoc, setDoc, deleteDoc, writeBatch } from "firebase/firestore";
import { initializeFirestore, uploadCatalog } from "./initFirestore";
...
const dynEmailAchats = "achat@epj-electricite.com";
...
export default function App() {
  ...
  if(view==="done"){
    ...
    const mailDest = o ? (o.extraEmail ? `${dynEmailAchats},${o.extraEmail}` : EMAIL_ACHATS) : EMAIL_ACHATS;
  ...
  if(view==="admin"&&user?.fonction==="Admin"){
    ...
    const adminInitAll = async () => {
      ...
      setAdminSaving(true);
      const r = await initializeFirestore();
      showT(r.message);
      if(r.users > 0) {
        // Also upload catalog
    ...
    if(adminSection==='catalog'){
      const cats = [...new Set(dynCatalog.map(p=>p.c))].sort();
      const [adminCatFilter, setAdminCatFilter] = [selectedCat, setSelectedCat];
      const [adminSearch, setAdminSearch] = [search, setSearch];
