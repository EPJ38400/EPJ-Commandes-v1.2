...
/* ═══════════════════════════════════════════════════
   EPJ — Application de Commande de Matériel V1.2
   Électricité Générale — 12 avril 2026
   ═══════════════════════════════════════════════════ */
...
export default function App() {
  ...
  const selectedChantierObj = useMemo(() => CHANTIERS.find(c=>c.nom===chantier), [chantier]);

  const allCategories = useMemo(() => [...new Set(CATALOG.map(p=>p.c))], []);
  const availableCategories = useMemo(() => orderType==="equipement" ? EQUIP_CATS : allCategories, [orderType, allCategories]);

  const filteredProducts = useMemo(() => {
    let items = CATALOG;
    if (orderType==="equipement") items = items.filter(p=>EQUIP_CATS.includes(p.c));
  ...
  const cartItems = useMemo(() => Object.entries(cart).filter(([,q])=>q>0).map(([r,q])=>{const p=CATALOG.find(x=>x.r===r);return p?{...p,qty:q}:null}).filter(Boolean), [cart]);
  ...
  const pendingCount = useMemo(() => {
    ...
    return pendingOrders.filter(o => {
      const ch = CHANTIERS.find(c=>c.nom===o.chantier);
  ...
  const doLogin = () => {
    const u = USERS.find(x=>x.id===loginId && x.pwd===loginPwd);
  ...
  const sendOrder = () => {
    ...
    const order = {
      ...
      chantierObj:chObj, numAffaire:chObj?.num||'',
      salarie:targetSalarie||`${user.prenom} ${user.nom}`,
      urgent, livraison, remarques, dateReception, statut, extraEmail, motifRefus:""
    };

    if(needsValidation){ setPendingOrders(prev=>[order,...prev]); }
    else {
      // Generate PDF and show it inline
      setPdfOrder(order);
      // Prepare mailto link (shown as button on done screen)
    }
    setHistory(prev=>[order,...prev]);
    setCmdCounter(prev=>prev+1);
    setView("done");
  };

  const validateOrder = (orderNum) => {
    const order = pendingOrders.find(o=>o.num===orderNum);
    if(!order) return;
    setPendingOrders(prev=>prev.filter(o=>o.num!==orderNum));
    const updated = {...order, statut:"Validée"};
    setHistory(prev=>prev.map(o=>o.num===orderNum?updated:o));
    setPdfOrder(updated);
    setView("pdfPreview");
    showT("✅ Commande validée — PDF généré");
  };

  const refuseOrder = (orderNum, motif) => {
    setPendingOrders(prev=>prev.filter(o=>o.num!==orderNum));
    setHistory(prev=>prev.map(o=>o.num===orderNum?{...o,statut:"Refusée",motifRefus:motif}:o));
  ...
  if(view==="login") return (
    <div style={{fontFamily:font,background:`linear-gradient(135deg,${EPJ.dark} 0%,#2a2a2a 100%)`,minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:20}}>
      ...
      <div style={{marginTop:20,fontSize:11,color:'rgba(255,255,255,.3)'}}>v1.2 — EPJ Commandes</div>
    </div>
  );
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              {availableCategories.map(cat=>(
                <div key={cat} onClick={()=>setSelectedCat(cat)} style={{background:'#fff',borderRadius:14,padding:'14px 10px',cursor:'pointer',textAlign:'center',fontSize:11,fontWeight:600,color:EPJ.dark,boxShadow:'0 1px 3px rgba(0,0,0,.04)',lineHeight:1.3,border:'2px solid transparent'}}>
                  <span style={{fontSize:26,display:'block',marginBottom:4}}>{CAT_ICONS[cat]||'📦'}</span>{cat}
                </div>
              ))}
            </div>
          ...
          {cartItems.length===0?<div style={{textAlign:'center',padding:'50px 20px',color:EPJ.gray}}><div style={{fontSize:40,marginBottom:8}}>🛒</div><div style={{fontWeight:600}}>Panier vide</div></div>:(
            <>
              {Object.entries(cg).map(([cat,items])=>(
                <div key={cat}>
                  <div style={{fontSize:11,fontWeight:700,color:EPJ.gray,textTransform:'uppercase',margin:'12px 0 6px'}}>{CAT_ICONS[cat]||'📦'} {cat}</div>
                  {items.map(it=>(
          ...
          {orderType==='chantier'?(
              ...
              <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:700,color:EPJ.gray,display:'block',marginBottom:4}}>CHANTIER *</label>
                {!showNewChantier?(<>
                  <select className="epj-input" value={chantier} onChange={e=>setChantier(e.target.value)} style={{cursor:'pointer'}}>
                    ...
                    {CHANTIERS.filter(c=>c.statut==='Actif').map(c=><option key={c.nom} value={c.nom}>[{c.num}] {c.nom}</option>)}
                  </select>
                  {/* N° affaire auto */}
            ...
            <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:700,color:EPJ.gray,display:'block',marginBottom:4}}>SALARIÉ DESTINATAIRE</label>
              <select className="epj-input" value={targetSalarie} onChange={e=>setTargetSalarie(e.target.value)} style={{cursor:'pointer'}}>
                ...
                {USERS.filter(u=>u.id!==user.id).map(u=><option key={u.id} value={`${u.prenom} ${u.nom}`}>{u.prenom} {u.nom}</option>)}
              </select>
            </div>
          )}
  ...
  if(view==="pending"){
    const fullName=`${user.prenom} ${user.nom}`;
    const myP=user.fonction==="Admin"?pendingOrders:pendingOrders.filter(o=>{const ch=CHANTIERS.find(c=>c.nom===o.chantier);return ch&&ch.conducteur===fullName});
    return(
      <div style={{fontFamily:font,background:EPJ.grayLight,minHeight:'100vh',maxWidth:520,margin:'0 auto'}}>
        <style>{css}</style>
