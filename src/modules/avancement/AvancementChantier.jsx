// ═══════════════════════════════════════════════════════════════
//  AvancementChantier v5 — écran détail
...
export function AvancementChantier({ chantier, onBack, canEdit, allUsers }) {
  ...
  // Qui peut éditer les tâches ? Admin, Direction, ou Conducteur du chantier
  ...
  // Mode édition des tâches (toggle global)
  ...
  // Progression locale (réactivité immédiate) + sauvegarde différée
  ...
  const saveProgress = (newProgress) => {
    ...
    debounceTimerRef.current = setTimeout(async () => {
      try {
        ...
        await updateDoc(doc(db, "chantiers", chantier.num), {
          avancementProgress: allProgress,
        });
      } catch (e) {
        console.error("Sauvegarde avancement:", e);
  ...
  // ─── Édition des tâches (ajouter, modifier libellé, supprimer, réorganiser) ──
  const saveTasksOverride = async (newCategories) => {
    // Construit l'override : on stocke les tâches modifiées par catégorie
    ...
    newCategories.forEach(cat => {
      if (cat.generated) return; // catégories générées (béton, placo) non modifiables ici
    ...
    try {
      await updateDoc(doc(db, "chantiers", chantier.num), {
        avancementTasksOverride: override,
      });
    } catch (e) {
      toast("❌ " + e.message);
    }
  ...
  const addTask = async (categoryId, label) => {
    ...
    const newCats = categories.map(cat => {
      ...
      return {
        ...cat,
        tasks: [...cat.tasks, { id: generateTaskId(categoryId), label: label.trim() }],
      };
    });
  ...
  const updateTaskLabel = async (categoryId, taskId, newLabel) => {
    ...
    const newCats = categories.map(cat => {
      ...
      return {
        ...cat,
        tasks: cat.tasks.map(t => t.id === taskId ? { ...t, label: newLabel.trim() } : t),
      };
    });
    await saveTasksOverride(newCats);
  };
  ...
  const deleteTask = async (categoryId, taskId) => {
    ...
    await saveTasksOverride(newCats);
    // Nettoie aussi la progression de cette tâche
  ...
  return (
    <div style={{ paddingTop: 12, paddingBottom: 24 }}>
      ...
      {editTasksMode && (
        <div className="epj-card" style={{ padding: "10px 12px", marginBottom: 10, fontSize: 11, color: EPJ.gray700, lineHeight: 1.5, background: `${EPJ.orange}10`, borderColor: `${EPJ.orange}44` }}>
          <strong style={{ color: EPJ.orange }}>Mode édition</strong> — vous pouvez ajouter, modifier, supprimer ou réordonner les tâches. Les tâches de <b>Béton</b> et <b>Placo</b> sont générées automatiquement et non modifiables ici (changez la typologie du bâtiment dans Admin → Chantiers).
        </div>
      )}
      ...
      {!editTasksMode && affectedArtisans.length > 0 && (
        <div style={{ marginBottom: 10 }}>
          <label style={{ display: "block", fontSize: 10, fontWeight: 600, color: EPJ.gray500, letterSpacing: 0.4, textTransform: "uppercase", marginBottom: 4 }}>Affichage</label>
...
function CategoryBlock({
  category, progress, onProgressChange, canEdit,
...
// ─── Ligne d'une tâche en mode normal ─────────────────────────
function TaskRow({ task, value, onChange, canEdit, affectedArtisans, assignedArtisanId, onAssignArtisan }) {
    ...
    <div style={{ padding: "10px 0", borderTop: `1px solid ${EPJ.gray100}` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {assignedArtisan && (
            <div style={{ fontSize: 10, color: EPJ.orange, marginTop: 2, fontWeight: 600, letterSpacing: 0.2 }}>
      ...
      {showArtisanPicker && (
        <div style={{ marginTop: 8, padding: "8px 10px", background: EPJ.gray50, borderRadius: 8, border: `1px solid ${EPJ.gray200}` }}>
...
// ─── Ligne d'une tâche en mode édition ────────────────────────
function EditableTaskRow({ task, isFirst, isLast, onUpdateLabel, onDelete, onMove }) {
  ...
  return (
    <div style={{
      padding: "8px 0", borderTop: `1px solid ${EPJ.gray100}`,
