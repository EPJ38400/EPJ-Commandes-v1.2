...
export function getCategoriesForConfig(cfg, tasksConfig, chantierOverride, buildingId) {
  ...
  let cats = FACTORY_CATEGORIES.map(cat => {
    if (cat.generated === "beton") return { ...cat, tasks: buildBetonTasks(cfg) };
    if (cat.generated === "placo") return { ...cat, tasks: buildPlacoTasks(cfg) };
    return cat;
  });
...
export function overallProgress(cfg, progressData, tasksConfig, chantierOverride, buildingId) {
  const cats = getCategoriesForConfig(cfg, tasksConfig, chantierOverride, buildingId);
  let sum = 0, count = 0;
