...
export function ParcOutilDetail({ outil, onBack }) {
  ...
  // Transfert : détenteur actuel OU gestionnaire (logique inchangée)
  const canTransfer = (isCurrentHolder || userCanGerer) && sortieEnCours;
  // Retour : tout le monde peut enregistrer un retour
  // (si Joseph sort la perceuse mais Thibaut la ramène à l'atelier, Thibaut doit pouvoir valider)
  const canRetour = !!sortieEnCours;

  // ── Détection pack ──
  const { outils: allOutils } = useData();
  // Si l'outil actuel est un enfant d'un pack maître actuellement sorti
  const parentPackSortie = useMemo(() => {
    if (!sortieEnCours || !sortieEnCours.packMaitreOutilId) return null;
    if (sortieEnCours.packMaitreOutilId === outil._id) return null; // c'est le maître lui-même
    const maitre = allOutils.find(o => o._id === sortieEnCours.packMaitreOutilId);
    return maitre ? { maitre, sortie: sortieEnCours } : null;
  }, [sortieEnCours, allOutils, outil._id]);

  // Si l'outil actuel est un pack maître sorti, trouver toutes les sorties enfants de ce pack
  const sortiesEnfantsPack = useMemo(() => {
    if (!sortieEnCours || !outil.isPack || !sortieEnCours.packSortieId) return [];
    return outillageSorties.filter(s =>
      s.packSortieId === sortieEnCours.packSortieId &&
      !s.dateRetourReelle &&
      s.outilId !== outil._id
    );
  }, [sortieEnCours, outillageSorties, outil]);
  ...
  return (
    <div style={{ paddingTop: 12, paddingBottom: 24 }}>
      ...
      {/* Badge "Fait partie du pack X" si outil enfant d'un pack sorti */}
      {parentPackSortie && (
        <div className="epj-card" style={{
          ...
          }}>🔗 Fait partie du pack</div>
          <div style={{ fontSize: 12, color: EPJ.gray900, fontWeight: 600 }}>
            📦 {parentPackSortie.maitre.nom}
          </div>
          <div style={{ fontSize: 10, color: EPJ.gray500, marginTop: 2, fontFamily: "monospace" }}>
            {parentPackSortie.maitre.ref}
          </div>
          <div style={{
            fontSize: 10, color: EPJ.gray600, marginTop: 6, lineHeight: 1.5,
          }}>
            Cet outil a été sorti dans le cadre d'un pack. Tu peux rendre chaque outil individuellement, ou rendre le pack complet depuis la fiche du pack maître.
          </div>
        </div>
      )}

      {/* Badge "Ce pack contient N outils actuellement sortis" si pack maître */}
      {outil.isPack && sortiesEnfantsPack.length > 0 && (
        <div className="epj-card" style={{
          padding: "10px 12px", marginBottom: 12,
          background: `${EPJ.orange}08`,
          borderLeft: `3px solid ${EPJ.orange}`,
          ...
          }}>📦 Pack en cours de sortie</div>
          <div style={{ fontSize: 12, color: EPJ.gray700, lineHeight: 1.5 }}>
            Ce pack contient <b>{sortiesEnfantsPack.length + 1} outil{sortiesEnfantsPack.length + 1 > 1 ? "s" : ""}</b> actuellement sorti{sortiesEnfantsPack.length + 1 > 1 ? "s" : ""} :
          </div>
          <div style={{ marginTop: 6, display: "flex", flexDirection: "column", gap: 3 }}>
            {sortiesEnfantsPack.map(s => (
              <div key={s.id} style={{
                ...
                <span style={{ fontFamily: "monospace", fontWeight: 700 }}>{s.ref}</span>
      ...
      {(effectiveStatus === "sorti" || effectiveStatus === "en_retard") && (
        <>
          {canRetour && outil.isPack && sortiesEnfantsPack.length > 0 && (
            <button
              onClick={() => {
                if (confirm(`Retour groupé du pack "${outil.nom}" (${sortiesEnfantsPack.length + 1} outils) ?\n\nTu vas valider le retour du pack maître puis des ${sortiesEnfantsPack.length} outil(s) associé(s), un par un.\nClique sur chaque fiche après validation.`)) {
                  setFlow("retour");
                }
              }}
              style={{
              ...
              📦 Retour du pack complet ({sortiesEnfantsPack.length + 1} outils)
            </button>
          )}
          {canRetour && (
          ...
          {!canTransfer && (
            <div className="epj-card" style={{
              ...
              💡 Seul le détenteur actuel ou un gestionnaire peut transférer cet outil à une autre personne.
            </div>
          )}
        </>
      )}
          ...
          {historiqueRendues.map(h => {
            ...
            return (
              <div key={h._id} className="epj-card" style={{
                ...
                {h.retourParNom && h.retourParUserId !== h.emprunteurId && (
                  <div style={{ fontSize: 10, color: EPJ.green, marginTop: 2, fontWeight: 600 }}>
                    ✅ Rentré par {h.retourParNom}
                  </div>
                )}
                {h.panneIds?.length > 0 && (
