...
export function ParcMateriels({ onSelectOutil }) {
  ...
  if (selectedCatId) {
    ...
    return (
      <div>
        ...
        {/* Bannière mode panier actif */}
        {panier.active && (
          <div style={{
            padding: "8px 10px", marginBottom: 10,
            background: `${EPJ.orange}12`,
            borderLeft: `3px solid ${EPJ.orange}`,
            borderRadius: 6,
            fontSize: 11, color: EPJ.gray700, lineHeight: 1.5,
          }}>
            🛒 <b style={{ color: EPJ.orange }}>Sélection multiple</b> — tape sur les outils disponibles pour les ajouter au panier ({panier.count} actuellement).
          </div>
        )}
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map(o => (
            <OutilCard
              ...
              panierMode={panier.active}
              isInPanier={panier.has(o._id)}
              onTogglePanier={() => panier.toggle(o._id)}
            />
          ))
        )}
      </div>
  ...
  return (
    <div>
      {/* Bouton démarrage sortie multiple (si utilisateur autorisé + pas déjà actif) */}
      {userCanSortir && !panier.active && enriched.some(o => computeOutilStatut(o, outillageSorties) === "disponible") && (
        <button
          onClick={panier.startPanier}
          style={{
        ...
        >
          🛒 Commencer une sortie de plusieurs outils
        </button>
      )}

      {/* Bannière d'information si panier actif */}
      {panier.active && (
        <div style={{
          ...
          <b style={{ color: EPJ.orange }}>Mode sélection multiple actif</b> — tape sur les outils <b>disponibles</b> pour les ajouter à ton panier.
          Tu peux naviguer entre les catégories, le panier garde ta sélection.
        </div>
      )}
...
function OutilCard({ outil, users, categories, onClick, showCategory, panierMode, isInPanier, onTogglePanier }) {
  ...
  // En mode panier : on ne peut cocher QUE les outils disponibles
  // Le clic sur la carte toggle l'ajout au panier
  // La zone "ouvrir fiche" devient le petit bouton (i) à droite
  const panierCanSelect = panierMode && isDisponible;

  return (
    <div
      className="epj-card"
      onClick={panierMode
        ? (panierCanSelect ? onTogglePanier : undefined)
        : onClick
      }
      style={{
