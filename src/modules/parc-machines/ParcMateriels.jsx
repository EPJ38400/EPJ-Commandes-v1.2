...
import {
  ...
  getCategorieIcon, getCategorieLabel, canSortirOutil,
} from "./parcUtils";
import { usePanierSortie } from "./PanierSortieContext";
import { PackExpandModal } from "./PackExpandModal";
...
export function ParcMateriels({ onSelectOutil }) {
  ...
  const [expandedPack, setExpandedPack] = useState(null); // outil pack en cours d'expansion
  const panier = usePanierSortie();
  const userCanSortir = canSortirOutil(user);

  // Gestion du toggle panier avec expansion du pack si besoin
  const handleTogglePanier = (outil) => {
    if (outil.isPack && !panier.has(outil._id)) {
      // Ajout d'un pack → ouvre la modal pour expansion
      setExpandedPack(outil);
    } else {
      // Toggle normal (outil simple ou retrait du panier)
      panier.toggle(outil._id);
    }
  };

  const handlePackConfirm = (outilIds) => {
    panier.addMany(outilIds);
  ...
  if (selectedCatId) {
    ...
    return (
      <div>
        {/* Modal d'expansion d'un pack */}
        {expandedPack && (
          <PackExpandModal
            pack={expandedPack}
            outils={outils}
            outillageSorties={outillageSorties}
        ...
        {/* Petit rappel panier si actif */}
        {panier.active && (
          <div style={{
            padding: "6px 10px", marginBottom: 10,
            background: `${EPJ.orange}10`, borderRadius: 6,
            fontSize: 10, color: EPJ.orange, fontWeight: 600,
            textAlign: "center",
          }}>
            🛒 {panier.count} outil{panier.count > 1 ? "s" : ""} dans ton panier — barre en bas pour valider
          </div>
        )}
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map(o => (
            <OutilCard
              ...
              panierMode={userCanSortir}
              isInPanier={panier.has(o._id)}
              onTogglePanier={() => handleTogglePanier(o)}
            />
          ))
        )}
      </div>
  ...
  return (
    <div>
      ...
      {/* Petit rappel visuel que les cases à cocher permettent la sortie multiple */}
      {userCanSortir && !panier.active && (
        <div style={{
          fontSize: 10, color: EPJ.gray500, lineHeight: 1.4,
          marginBottom: 10, padding: "6px 10px",
          background: EPJ.gray50, borderRadius: 6,
          textAlign: "center",
        }}>
          💡 Coche les outils à sortir (un ou plusieurs) — barre de validation en bas dès 1 sélection
        </div>
      )}
...
function OutilCard({ outil, users, categories, onClick, showCategory, panierMode, isInPanier, onTogglePanier }) {
  ...
  const isDisponible = outil.effectiveStatus === "disponible";

  // v10 : cases à cocher TOUJOURS visibles pour les outils dispo si l'utilisateur peut sortir
  // Clic sur la case = toggle panier / Clic sur le reste = ouvrir la fiche détail
  const showCheckbox = panierMode && isDisponible;
  ...
  return (
    <div
      ...
      style={{
        ...
        borderLeft: `3px solid ${isInPanier ? EPJ.orange : st.color}`,
        cursor: "pointer",
        background: isInPanier ? `${EPJ.orange}0A` : undefined,
        outline: isInPanier ? `2px solid ${EPJ.orange}60` : undefined,
        transition: "background .15s ease, border-color .15s ease",
      }}
    >
      {/* Case à cocher (toujours visible pour les outils dispo en mode panier) */}
      {showCheckbox && (
        <div
          ...
          title={isInPanier ? "Retirer du panier" : "Ajouter au panier"}
        >{isInPanier ? "✓" : ""}</div>
      )}
        ...
        <div style={{
          ...
          {outil.isPack && (
            <span style={{
              ...
              background: `${EPJ.orange}18`, color: EPJ.orange,
              padding: "2px 6px", borderRadius: 4,
              fontFamily: "Inter, sans-serif", letterSpacing: 0.3,
            }}>📦 PACK {Array.isArray(outil.packContent) ? `(${outil.packContent.length})` : ""}</span>
          )}
        </div>
        <div style={{
