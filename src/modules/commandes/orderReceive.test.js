...
function computeReliquatItems(items, receivedByIndex) {
  ...
  items.forEach((it, idx) => {
    ...
    if (missing > 0) out.push(Object.assign({}, it, { qte: missing }));
  });
  return out;
}
...
// getReceptionKind
const items3 = [{ qte: 10 }, { qte: 5 }, { qte: 2 }];
...
// computeReliquatItems
{
  const r = computeReliquatItems([{ ref: "A", qte: 10 }, { ref: "B", qte: 5 }], { 0: 7, 1: 5 });
  assertEq(r.length, 1, "reliquat: 1 ligne (B complet)");
  assertEq(r[0].ref, "A", "reliquat: ref A préservée");
  assertEq(r[0].qte, 3, "reliquat: qte = manquant");
}
{
  const r = computeReliquatItems([{ ref: "A", qte: 10 }], { 0: 10 });
  assertEq(r.length, 0, "reliquat: rien si tout reçu");
}
{
  const r = computeReliquatItems([{ ref: "A", qte: 10 }], {});
  assertEq(r[0].qte, 10, "reliquat: rien reçu → reliquat = commande entière");
}

// validateReceivedQuantities
assertEq(validateReceivedQuantities([{ qte: 5 }], { 0: 3 }).valid, true, "validate: 3<5 OK");
assertEq(validateReceivedQuantities([{ qte: 5 }], { 0: 5 }).valid, true, "validate: 5=5 OK");
assertEq(validateReceivedQuantities([{ qte: 5 }], { 0: 6 }).valid, false, "validate: 6>5 KO");
assertEq(validateReceivedQuantities([{ qte: 5 }], { 0: 6 }).errors.length, 1, "validate: 1 erreur");
assertEq(validateReceivedQuantities(null, {}).valid, false, "validate: items null → invalide");

// buildReceptionPayload — mode "tout reçu"
{
  const order = { items: [{ qte: 10 }, { qte: 5 }] };
...
{
  const order = { items: [{ ref: "A", qte: 10 }, { ref: "B", qte: 5 }] };
  ...
  assertEq(p.reliquatItems[0].qte, 3, "payload détail: qte = manquant");
}
