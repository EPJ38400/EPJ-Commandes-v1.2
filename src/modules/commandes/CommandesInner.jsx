...
// v10.E — C4 : `directAchat` contrôle si la commande passe en validation.
//   • directAchat:true  → la commande part directement aux achats sans validation
//   • directAchat:false → la commande passe par "En attente de validation"
// Règle EPJ : SEULE la Direction (admin) est `directAchat:true`. Tous les
// conducteurs de travaux (Joseph, Guillaume, Thibaut, Mickaël) ont leurs
// commandes soumises à validation.
const USERS = [
  { id:"admin", pwd:"admin2010", prenom:"Admin", nom:"EPJ", fonction:"Admin", email:"admin@epj.fr", directAchat:true },
  { id:"Bilardo", pwd:"1234", prenom:"Joseph", nom:"BILARDO", fonction:"Conducteur de travaux", email:"j.bilardo@epj-electricite.com", directAchat:false },
...
const CATALOG = [
  ...
  // ─── v10.E — C2 : nouvelle catégorie "Audace + Ovalis" (gammes Schneider) ───
  // Placeholders pour faire apparaître la catégorie + les 2 sous-catégories.
  // Pierre-Julien y rattachera ses vraies références via Admin → Articles.
  {c:"Audace + Ovalis",s:"Audace",r:"_PLACEHOLDER_AUDACE_",n:"(Catégorie Audace — ajouter vos références ici)",u:"Pièce",stock:false},
  {c:"Audace + Ovalis",s:"Ovalis",r:"_PLACEHOLDER_OVALIS_",n:"(Catégorie Ovalis — ajouter vos références ici)",u:"Pièce",stock:false},
    ...
    <div style={{fontFamily:font,background:'transparent',minHeight:'100vh',maxWidth:520,margin:'0 auto',paddingBottom:80}}>
      ...
      {showDivers&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:300,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={()=>setShowDivers(false)}>
        <div style={{background:'#fff',borderRadius:'20px 20px 0 0',padding:'24px 20px 30px',width:'100%',maxWidth:520}} onClick={e=>e.stopPropagation()}>
          ...
          <button className="epj-btn" onClick={()=>{if(diversName.trim()){const ref=diversRef.trim()||`DIV-${Date.now()}`;const newArticle={c:'Divers',s:'Article libre',r:ref,n:diversName.trim(),u:'Pièce'};CATALOG.push(newArticle);setDynCatalog(prev=>[...prev,newArticle]);setCart(p=>({...p,[ref]:diversQty}));setDiversName('');setDiversRef('');setDiversQty(1);setShowDivers(false);showT('✓ Ajouté au panier')}}} style={{width:'100%',background:EPJ.blue,color:'#fff'}} disabled={!diversName.trim()}>Ajouter au panier</button>
        </div>
      </div>}
      <QtyPopupOverlay/>
    </div>
