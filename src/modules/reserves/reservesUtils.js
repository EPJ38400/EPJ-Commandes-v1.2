...
// ─── Signature utilisateur (stockée dans le profil) ──────
// Upload une signature PNG dans Firebase Storage → dossier signatures/
export async function uploadUserSignature(userId, pngDataUrl) {
  if (!pngDataUrl || !pngDataUrl.startsWith("data:image/")) {
    throw new Error("Signature invalide");
  }
  // Convertit dataURL → blob
  const res = await fetch(pngDataUrl);
  const blob = await res.blob();
  const safeId = String(userId || "user").replace(/[\/\s]/g, "_");
  const path = `signatures/${safeId}_${Date.now()}.png`;
  const fileRef = ref(storage, path);
...
export async function deleteUserSignature(path) {
  if (!path) return;
  try {
    const fileRef = ref(storage, path);
    await deleteObject(fileRef);
  } catch (e) {
    console.warn("deleteUserSignature (non bloquant):", e.message);
  }
}

// ─── Upload PDF de quitus généré ──────────────────────────
export async function uploadQuitusPdf(reserveId, blob) {
  const safeId = String(reserveId || "res").replace(/[\/\s]/g, "_");
  const path = `quitus/${safeId}_${Date.now()}.pdf`;
  const fileRef = ref(storage, path);
...
// ─── Génère le PDF du quitus en mémoire (Blob) ───────────────
// Utilise un iframe caché + html2canvas + jsPDF, puis récupère le Blob.
// Sert pour l'upload automatique vers Firebase Storage (bouton Email/Lien).
export async function generateQuitusPdfBlob(opts) {
  // Import dynamique du HTML builder (évite cycle si nécessaire)
  const mod = await import("./quitusPdfGenerator.js");
  const html = mod.buildQuitusHtml(opts);

  // Crée un iframe invisible
  const iframe = document.createElement("iframe");
  iframe.style.cssText = "position:fixed;left:-10000px;top:0;width:900px;height:1300px;border:0;visibility:hidden;";
  document.body.appendChild(iframe);

  // Charge les scripts html2canvas + jsPDF dans le document parent si pas déjà là
  ...
  try {
    // Écrit le HTML dans l'iframe
    iframe.contentDocument.open();
    iframe.contentDocument.write(html);
    iframe.contentDocument.close();

    // Attend le chargement complet (images comprises)
    ...
    const page = iframe.contentDocument.getElementById("quitus-page");
    if (!page) throw new Error("Rendu PDF : élément introuvable");

    // html2canvas est dans le contexte parent, on lui passe l'élément de l'iframe
...
// Helper : charge un script dans le document principal, une seule fois
