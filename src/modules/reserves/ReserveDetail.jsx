    ...
    <div style={{ paddingTop: 12, paddingBottom: 40 }}>
      ...
      {["levee", "partiellement_levee", "quitus_signe"].includes(reserve.statut) && (
        <div className="epj-card" style={{ padding: 14, marginBottom: 10, borderLeft: `3px solid ${EPJ.green}` }}>
          ...
          {/* Bouton génération quitus PDF */}
          <button
            onClick={() => {
              const chantier = chantiers.find(c => c._id === reserve.chantierId);
              const technicien = users.find(u => u._id === reserve.leveePar);
              openQuitusWindow({
                reserve,
                chantier,
                company,
          ...
          {!company.papierEnteteUrl && (
            <div style={{
              fontSize: 10, color: EPJ.orange, marginTop: 6,
