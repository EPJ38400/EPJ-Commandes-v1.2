...
export function openQuitusWindow({
  ...
  // Mention RGPD (petit texte en bas du quitus)
  const mentionRgpd = co.mentionRgpd || "";
...
<body class="notranslate" translate="no">
  <div id="quitus-page">
    ...
    <div class="content">
      ...
      ${mentionRgpd ? `<div class="rgpd-note">${escapeHtml(mentionRgpd)}</div>` : ""}
...
// ═══════════════════════════════════════════════════════════════
// buildQuitusHtml
// Produit un document HTML autonome (sans boutons, sans scripts)
// destiné à être rendu dans un iframe caché pour génération PDF
// automatique (upload vers Firebase → envoi email, copie lien).
// ═══════════════════════════════════════════════════════════════
export function buildQuitusHtml({
  reserve, chantier, company, technicien,
  clientSigPng, clientNom, clientQualite,
}) {
  const co = company || {};
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #quitus-page {
      ...
      margin: 0; background: #ffffff; position: relative; overflow: hidden;
    }
    .bg-papier { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; z-index: 0; }
    .content { position: relative; z-index: 1; padding: 150px 52px 140px 52px; }
    .title-block { text-align: right; margin-bottom: 26px; }
    .title-main { font-size: 22px; font-weight: 800; color: #00A3E0; line-height: 1.1; letter-spacing: -0.02em; }
    .title-sub { font-size: 14px; font-weight: 700; color: #4A4A4A; margin-top: 8px; font-family: monospace; }
    .title-date { font-size: 11px; color: #8E8E8E; margin-top: 4px; }
    .section-title { display: flex; align-items: center; gap: 10px; margin-top: 14px; margin-bottom: 8px; }
    .section-title-bar { width: 22px; height: 3px; background: #00A3E0; border-radius: 2px; }
    .section-title-text { font-size: 11px; font-weight: 800; color: #00A3E0; letter-spacing: 1px; text-transform: uppercase; }
    .section-title-line { flex: 1; height: 1px; background: #E5E5E5; }
    .kv { display: grid; grid-template-columns: 150px 1fr; gap: 4px 12px; margin-left: 6px; }
    .kv-label { font-size: 11px; color: #8E8E8E; }
    .kv-value { font-size: 11px; font-weight: 700; color: #1A1A1A; }
    .photos-row { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin: 8px 6px; }
    .photo-card { border: 2px solid #D4D4D4; border-radius: 6px; overflow: hidden; background: #fff; }
    .photo-card.avant { border-color: #E53935; }
    .photo-card.apres { border-color: #A8C536; }
    .photo-card img { width: 100%; height: 140px; object-fit: cover; display: block; }
    .photo-card-label { padding: 4px 8px; font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
    .photo-card.avant .photo-card-label { background: #E53935; color: #fff; }
    .photo-card.apres .photo-card-label { background: #A8C536; color: #fff; }
    .decl-box { margin: 14px 6px 0; background: #F0F9FD; border: 1.5px solid #00A3E0; border-radius: 8px; padding: 12px 14px; }
    .decl-title { font-size: 11px; font-weight: 800; color: #00A3E0; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 8px; }
    .decl-text { font-size: 10.5px; line-height: 1.45; color: #1A1A1A; margin-bottom: 6px; }
    .decl-lieu { font-size: 10px; font-style: italic; color: #4A4A4A; margin-bottom: 10px; }
    .sig-row { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    .sig-box { border: 1px solid #D4D4D4; border-radius: 6px; padding: 8px; background: #fff; min-height: 80px; position: relative; }
    .sig-label { font-size: 9px; font-weight: 800; color: #00A3E0; letter-spacing: 0.5px; text-transform: uppercase; margin-bottom: 4px; }
    .sig-image { width: 100%; height: 50px; object-fit: contain; display: block; margin: 2px 0; }
    .sig-name { font-size: 9px; color: #4A4A4A; font-weight: 600; }
    .priorite-pill { display: inline-block; font-size: 10px; font-weight: 800; padding: 2px 8px; border-radius: 10px; color: #fff; margin-left: 4px; }
    .description { font-size: 11px; line-height: 1.4; margin-left: 6px; margin-top: 4px; padding: 6px 8px; background: #F5F5F5; border-radius: 4px; }
    .rgpd-note { font-size: 8px; color: #8E8E8E; line-height: 1.35; margin: 8px 6px 0; padding: 6px 8px; border-top: 1px solid #E5E5E5; font-style: italic; }
  </style>
</head>
<body class="notranslate" translate="no">
  <div id="quitus-page">
    ...
    <div class="content">
      ...
      <div class="section-title"><span class="section-title-bar"></span><span class="section-title-text">Chantier</span><span class="section-title-line"></span></div>
      <div class="kv">
        <div class="kv-label">N° d'affaire</div><div class="kv-value">${escapeHtml(reserve.chantierNum || "—")}</div>
        <div class="kv-label">Nom du chantier</div><div class="kv-value">${escapeHtml(reserve.chantierNom || "—")}</div>
        <div class="kv-label">Adresse</div><div class="kv-value">${escapeHtml(reserve.chantierAdresse || chantier?.adresse || "—")}</div>
        <div class="kv-label">Emplacement</div><div class="kv-value">${escapeHtml(emplacementStr)}</div>
      </div>

      <div class="section-title"><span class="section-title-bar"></span><span class="section-title-text">Descriptif de la réserve</span><span class="section-title-line"></span></div>
      <div class="kv">
        <div class="kv-label">Émise par</div><div class="kv-value">${escapeHtml(emisParFull)}</div>
        <div class="kv-label">Date d'émission</div><div class="kv-value">${escapeHtml(fmt(reserve.dateEmission))}</div>
        <div class="kv-label">Priorité</div>
        <div class="kv-value">
          <span class="priorite-pill" style="background:${prioriteColor}">${prioriteLabel}</span>
        </div>
      ...
      ${(reserve.photoAvant || reserve.photoApres) ? `
      <div class="section-title"><span class="section-title-bar"></span><span class="section-title-text">Constat &amp; Reprise</span><span class="section-title-line"></span></div>
      <div class="photos-row">
        ${reserve.photoAvant ? `<div class="photo-card avant"><img src="${escapeHtml(reserve.photoAvant)}" crossorigin="anonymous" alt="avant"/><div class="photo-card-label">▸ Avant — Constat</div></div>` : `<div></div>`}
        ${reserve.photoApres ? `<div class="photo-card apres"><img src="${escapeHtml(reserve.photoApres)}" crossorigin="anonymous" alt="après"/><div class="photo-card-label">▸ Après — Reprise réalisée</div></div>` : `<div></div>`}
      </div>
      ` : ""}

      <div class="section-title"><span class="section-title-bar"></span><span class="section-title-text">Intervention</span><span class="section-title-line"></span></div>
      <div class="kv">
        <div class="kv-label">Technicien EPJ</div><div class="kv-value">${escapeHtml(technomFull)}</div>
        <div class="kv-label">Date d'intervention</div><div class="kv-value">${escapeHtml(fmt(reserve.dateLevee))}</div>
      </div>
      <div class="description">${escapeHtml(travaux)}</div>

      <div class="section-title"><span class="section-title-bar"></span><span class="section-title-text">Client final (occupant)</span><span class="section-title-line"></span></div>
      <div class="kv">
        <div class="kv-label">Nom</div><div class="kv-value">${escapeHtml(reserve.clientFinal?.nom || "—")}</div>
        ${reserve.clientFinal?.telephone ? `<div class="kv-label">Téléphone</div><div class="kv-value">${escapeHtml(reserve.clientFinal.telephone)}</div>` : ""}
        ${reserve.clientFinal?.email ? `<div class="kv-label">Email</div><div class="kv-value">${escapeHtml(reserve.clientFinal.email)}</div>` : ""}
      </div>
