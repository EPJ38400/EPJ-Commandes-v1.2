...
export function Layout({ children, currentModule, onHome, onOpenAdmin, fullWidth = false }) {
...
function Header({ user, currentModule, onHome, onLogout, onOpenAdmin }) {
  ...
  return (
    <header style={{
      ...
      // Safe-area iPhone : env() + fallback explicite pour PWA iOS
      // Si env() ne s'applique pas (cas où viewport-fit=cover manque dans index.html),
      // on a quand même un minimum pour ne pas chevaucher la barre de statut
