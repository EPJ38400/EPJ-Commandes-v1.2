...
export function HomePage({ onOpenModule, onOpenDashboard }) {
  ...
  const notifications = useMemo(() => {
    ...
    // v10.D — Notifications COMMANDES pour conducteurs
    const safeCommandes = commandes || [];
    const uid = user?._id || user?.id;

    // Est-ce un rôle pilotage (voit tout) ?
    const roles = [
    ...
    const isConducteur = roles.some(r => String(r).toLowerCase().includes("conducteur"));

    // Détermine quelles commandes concernent l'utilisateur
    const isMyCommande = (cmd) => {
      if (isPilotage) return true;
      if (!isConducteur) return false;
      // Un conducteur voit les commandes de SES chantiers
      if (!cmd.chantier) return false;
      const monChantier = chantiers.find(c =>
        c.nom === cmd.chantier &&
        (c.affectations?.conducteurId === uid)
      );
      return !!monChantier;
    };
