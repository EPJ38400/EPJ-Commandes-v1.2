...
export function AdminOutillage({ onBack }) {
  ...
  const noSeed = outils.length === 0 && outillageCategories.length === 0 && outillagePannes.length === 0;
  ...
  return (
    <div style={{ paddingTop: 12, paddingBottom: 24 }}>
      ...
      {/* Bloc import initial (seulement si tout est vide) */}
      {noSeed && (
        <div className="epj-card" style={{
          ...
          <div style={{ fontSize: 11, color: EPJ.gray700, lineHeight: 1.5, marginBottom: 10 }}>
            Le module Parc machines est vide. Importe le catalogue initial (223 outils EPJ, 18 catégories, 8 pannes récurrentes) pour démarrer rapidement.
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <button onClick={importCategoriesInitiales} disabled={!!importing || outillageCategories.length > 0} style={seedBtnStyle}>
              {importing === "categories" ? "⏳ Import…" : `1. Importer les 18 catégories (${outillageCategories.length > 0 ? "✓" : "à faire"})`}
            </button>
            <button onClick={importPannesInitiales} disabled={!!importing || outillagePannes.length > 0} style={seedBtnStyle}>
              {importing === "pannes" ? "⏳ Import…" : `2. Importer les 8 pannes récurrentes (${outillagePannes.length > 0 ? "✓" : "à faire"})`}
            </button>
            <button onClick={importOutilsInitiaux} disabled={!!importing || outils.length > 0 || outillageCategories.length === 0} style={seedBtnStyle}>
              {importing === "outils" ? "⏳ Import 223 outils en cours…" : `3. Importer les 223 outils EPJ (${outils.length > 0 ? "✓" : "à faire"})`}
            </button>
          </div>
          <div style={{ fontSize: 10, color: EPJ.gray500, marginTop: 8, lineHeight: 1.4 }}>
            Fais-les dans l'ordre. Une fois les catégories importées, tu peux importer les 223 outils.
          </div>
        </div>
      )}

      {/* Boutons import partiel si quelque chose manque */}
      {!noSeed && (outillageCategories.length === 0 || outillagePannes.length === 0) && (
        <div className="epj-card" style={{
          padding: 12, marginBottom: 12,
          background: `${EPJ.blue}08`,
          borderLeft: `3px solid ${EPJ.blue}`,
        }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: EPJ.gray900, marginBottom: 8 }}>
            Import partiel disponible :
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {outillageCategories.length === 0 && (
              <button onClick={importCategoriesInitiales} disabled={!!importing} style={seedBtnStyle}>
                {importing === "categories" ? "⏳…" : "Importer les 18 catégories"}
              </button>
            )}
            {outillagePannes.length === 0 && (
              <button onClick={importPannesInitiales} disabled={!!importing} style={seedBtnStyle}>
                {importing === "pannes" ? "⏳…" : "Importer les 8 pannes récurrentes"}
              </button>
            )}
          </div>
        </div>
