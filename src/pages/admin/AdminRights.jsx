...
import {
  ...
  DEFAULT_PERMISSIONS, getRoles, SCOPE_LABELS,
...
export function AdminRights({ onBack, focusedUserId, onClearFocus }) {
  ...
  // Scope par défaut calculé à partir des rôles de l'user
  // (on cumule les rôles, on prend le plus permissif)
  const getDefaultScope = (module, action) => {
    const roles = getRoles(user);
    let best = false;
    const rank = { "all":3, "own_chantiers":2, "own_items":1, false:0 };
    for (const r of roles) {
      const s = DEFAULT_PERMISSIONS[r]?.[module]?.[action];
    ...
    <div style={{ paddingTop: 12, paddingBottom: 24 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        <button
          ...
          style={{ background: EPJ.gray100, border: "none", borderRadius: 8, padding: "8px 12px", fontSize: 13, fontWeight: 600, color: EPJ.gray700, cursor: "pointer", fontFamily: font.body }}
        >←</button>
      ...
      <AdminToggle
        ...
        value={override._admin !== undefined ? override._admin : (DEFAULT_PERMISSIONS[getRoles(user)[0]]?._admin === true)}
      ...
      {/* Dashboards */}
      <DashboardsEditor override={override} setOverride={setOverride} user={user}/>
...
function DashboardsEditor({ override, setOverride, user }) {
  const roles = getRoles(user);
  const getDefault = (dash) => roles.some(r => DEFAULT_PERMISSIONS[r]?._dashboards?.[dash] === true);
